...
Illegal values are unconstructible rather than rejected at encode time. A map
sorts into the protocol's canonical key order in `FateMap::new` and refuses a
key containing a map; a variant checks its tag against its arity list in
`FateVariant::new`. `serialize` therefore has exactly one failure mode — a
typerep whose tuple or variant has more than 255 members, which the format
cannot express.
