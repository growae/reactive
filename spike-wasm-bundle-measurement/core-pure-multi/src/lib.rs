...
impl Guest for Component {
    ...
    // Second export, deliberately trivial: this function's own logic is not
    // the point — it exists to see whether a second export shares the jco
    // runtime/canonical-ABI glue with the first or duplicates it.
    fn decode_transfer_length(encoded: Vec<u8>) -> u32 {
        encoded.len() as u32
    }
}
