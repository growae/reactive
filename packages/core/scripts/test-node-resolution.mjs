...
// rather than let it stay invisible — and say so when it is not the version
// this repository pins, because that is when a local result and a CI result
...
console.log(
  `node ${process.version} · pnpm ${pnpmVersion} · started via ${
    process.env.npm_lifecycle_event
      ? `pnpm run ${process.env.npm_lifecycle_event}`
      : 'a direct node invocation'
  }`,
)
if (pinnedPackageManager !== `pnpm@${pnpmVersion}`) {
  console.log(
    `  note: this repository pins ${pinnedPackageManager}; the guard below does not depend on the version, but resolution details can.`,
  )
}
...
// The new home. Written as JSON, which is valid YAML, so no quoting rule about
// `@` or `file:` can silently produce a file pnpm reads as empty.
writeFileSync(
